...
write.dataset.biom <- function(ps, filePATH, filePREFIX, writeFASTA=TRUE, rename=FALSE, useREFSEQ=TRUE){
...
write.dataset <- function(ps, filePATH, filePREFIX, writeFASTA=TRUE, rename=FALSE, useREFSEQ=TRUE){
  ...
  tax.tab$taxonomy <- tidyr::unite(tax.tab, "out", c(colnames(tax.tab)), sep = ";")
